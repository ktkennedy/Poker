...
def equity(hole, board, num_opponents, trials=20000, seed=0):
    """hero(hole) vs num_opponents명 '랜덤' 상대의 에쿼티(0~1) 추정.
    승리=1.0, 공동 최강(찹)=1/공유인원, 패배=0. 매판 보드를 5장까지 채워 7장으로 평가."""
    ...
    for _ in range(trials):
        ...
        opp_ranks = [_EVALUATOR.evaluate(full_board, _to_treys(oh)) for oh in opp_holes]
        best_opp = min(opp_ranks)
        if hero_rank < best_opp:
            total += 1.0
        elif hero_rank == best_opp:
            sharers = 1 + sum(1 for r in opp_ranks if r == hero_rank)
            total += 1.0 / sharers
    return total / trials
